...
function duplicateResources($Ressourcelinks, $langContext, $modx) {
    $resource = $modx->getObject('modResource', $Ressourcelinks['id']);

    if ($resource) {
        // Dupliziere die Ressource in den neuen Kontext
        $newResource = $resource->duplicate(array(
        'context_key' => $langContext->get('key'), // Setze den neuen Kontext als Kontext der neuen Ressource
        'menu_index'=> ''
        ));
    }
}
